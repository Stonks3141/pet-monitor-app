...
pub async fn store<P: AsRef<Path>>(path: &Option<P>, ctx: &Context) -> anyhow::Result<()> {
    ...
    if let Some(path) = path {
        ...
        spawn_blocking(move || {
            confy::store("pet-monitor-app", None, ctx).context("Failed to store configuration file")
        })
        .await?
    }
}
...
pub async fn load<P: AsRef<Path>>(path: &Option<P>) -> anyhow::Result<Context> {
    ...
    if let Some(path) = path {
        ...
        spawn_blocking(move || {
            confy::load("pet-monitor-app", None).context("Failed to load configuration file")
        })
        .await?
    }
}
